...
generate_instrument_section <- function(result, metadata, section_index) {
  ...
  add("ident_", prefix, " <- tryCatch(read_csv(\"", dl, "/", name, "_identifiables.csv\", ", tpl_csv)
  add("pseudo_", prefix, " <- tryCatch(read_csv(\"", dl, "/", name, "_pseudonymises.csv\", ", tpl_csv)
  add("anon_", prefix, " <- tryCatch(read_csv(\"", dl, "/", name, "_anonymises.csv\", ", tpl_csv)
  tpl_csv_stats <- "show_col_types = FALSE), error = function(e) tibble(categorie = character(), nombre = integer()))"
  add("stats_", prefix, " <- tryCatch(read_csv(\"", dl, "/", name, "_statistiques.csv\", ", tpl_csv_stats)
  add("")
